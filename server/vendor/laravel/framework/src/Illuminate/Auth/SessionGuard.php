...
class SessionGuard implements StatefulGuard, SupportsBasicAuth
{
    ...
    public function owner()
    {
        ...
        // one exists. Otherwise we will check for a "remember me" cookie in this
        // request, and if one exists, attempt to retrieve the user using that.
        if (! is_null($id) && $this->owner = $this->provider->retrieveById($id)) {
            $this->fireAuthenticatedEvent($this->owner);
        }

        // If the user is null, but we decrypt a "recaller" cookie we can attempt to
        // pull the user data on that cookie which serves as a remember cookie on
        // the application. Once we have a user we can return it to the caller.
        if (is_null($this->owner) && ! is_null($recaller = $this->recaller())) {
            $this->owner = $this->userFromRecaller($recaller);

            if ($this->owner) {
                $this->updateSession($this->owner->getAuthIdentifier());

                $this->fireLoginEvent($this->owner, true);
            }
        }
    ...
    /**
     * Log the given user ID into the application without sessions or cook
     *
     * @param  mixed  $id
